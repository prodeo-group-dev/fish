...
| BK-PLT-8 | Real institution↔School linkage (EA auto-provisioning) | M | **Done, both sides.** SchoolAdmissions (`fish-school-admissions` `57ad955`) built by this thread, 2026-09-22. **Correction, 2026-09-23, from FiSH+ER WEB**: EA's own calling side (`fish-enterprise-administration` `4243574`) and its diagnostic-route verification against real production Cognito are WEB's own pre-existing work, not built in response to this thread's proposal as earlier reported here — this thread only confirmed the contract matched, didn't cause it to be built. State unchanged either way: genuinely live, genuinely verified end to end. Design: `docs/SchoolAdmissions_Institution_Linkage_And_Guardian_Billing_Design.md`. One cleanup item: a real "EA Verification Test - DELETE ME" School row (`ee7fc00d-7375-4538-98f9-3444cdde2dd1`) - Femi confirmed leave it as-is until real schools go live. Replaces WEB's Student Management Console manual-paste-School-ID stopgap. Unblocked Guardian-as-Customer - see BK-FEE row below. |
| BK-FEE-5 | Guardian-as-Customer: real SOP `Customer` for billing rollup | M | **Done 2026-09-23** (`fish-school-admissions` `c01b94d`; SOP's first-ever inbound service-account caller, `fish-sales-order-processing` `69a0d6e`). Direct instruction: "a Guardian is equivalent to a Customer... it is a contract the school has to fulfil." New `GuardianAccount` aggregate (additive, not a migration of the existing per-student embedded `Guardian` contact list - `relationship` stays on that per-student record, a real correction found while building: it belongs to the (Guardian, Student) pairing, not a shared identity). Deduped by (school, email-or-phone) via a real case-insensitive unique index. `FeeService.resolveGuardianCustomer` (new, this class's only `suspend` method) calls SOP's `POST /customers` lazily, first-billing-event only, using BK-PLT-8's `organisationId` as SOP's `entityId`. Full unit + real-Postgres integration coverage (3x rerun) on SchoolAdmissions; SOP's own new service-account bypass has dedicated end-to-end tests. **Not yet deployed to production** - needs new Cognito app clients + Terraform on both sides before the real env vars have values. |
...
| BK-ATT-2 | FR-ATT-3 | Government-mandated attendance codes only; reject others | M | **Done, 2026-09-22** (`fish-school-admissions` `8170adf`). `AttendanceCode` is now the 9-value UK DfE-style table (`/ L N I M C V B E`, `docs/BK-ATT-2_Mark_Code_Table_Proposal.md`), display/statutoryCategory/countsInPercent, adopted as real seed data per direct instruction ("go with the flow of the UK system as at now... if and when there is a Nigerian code, we will separate the education jurisdictions") — mirrors this project's own tax-jurisdiction UK/IE/NG/SL/LR/GN/CI precedent. **O4 stays open** (no real Nigerian source attached yet) but no longer blocks the platform. |
...
| BK-CUR-2 | Learning roadmap / scheme-of-work progress vs curriculum map | S | **Done 2026-09-21** (`74c31fd`). New `TopicProgressEntry` (one current status per school/class-section/topic, no change history) + `curriculum_topic_progress` table. `CurriculumService.recordProgress()`/`roadmapFor()` — the roadmap is BK-CUR-1's static map paired with each topic's real progress for a class section, `NOT_STARTED` where nothing's recorded. Same `canEnterGrades` gate as BK-CUR-1. |
| BK-CUR-3 | Feed curriculum coverage + progress into SPI/TPI inputs | M | **Done 2026-09-21** (`74c31fd`), the input surface only — real SPI/TPI computation stays BLK on O1. `CurriculumService.curriculumCoverageFor()` computes a real fraction-of-topics-completed signal; `SpiPlaceholder`/`TpiPlaceholder` gain an acknowledged-but-unused `curriculumCoverage` parameter (same idiom as the existing `grades` parameter) so the formula's input exists once O1 lands — output stays `null`. **Deliberately not wired into `AssessmentService.publish()`**: doing so would require correlating `GradeEntry.term` (e.g. `"2026-T1"`) against `CurriculumTopic`'s `academicYear`/`term` pair (e.g. `"2026/27"`/`"Term 1"`) — no such mapping exists anywhere in this codebase. Parked as a real open question for whoever builds BK-ASS-5/6, not guessed. |
| BK-CUR-4 | Teacher view: roadmap adherence for own subjects/classes | S | **Done 2026-09-21** (`f2d16dd`). Resolves the "academic-track RBAC" question BK-CUR-1 flagged, scoped to exactly this feature: new `ErPrincipal.canViewAnyClassRoadmap()` (SCHOOL_ADMIN/HEAD_TEACHER/HOD/HOY see any class); a plain `TEACHER` is now checked against real `LessonSlot` rows via `SchoolStore.lessonsForSection()` — the only existing source of teacher-to-class-subject assignment in this codebase, not a picked option. `CurriculumService.roadmapForTeacher()` tightens the existing `GET .../curriculum/roadmap/...` route rather than adding a parallel bypassable one; `roadmapFor()` itself stays principal-free for internal callers like `curriculumCoverageFor()`. |
| BK-CUR-5 | Per-occurrence lesson-to-curriculum scheduling + teacher session roster | S | **Done 2026-09-22** (`9ca801b`), on direct instruction — three explicit requirements: lessons/teaching sessions scheduled to cover the curriculum; teacher and students of a session known; teachers fed expected attendees of their teaching session. New `domain/classroom/ScheduledLesson` (natural key `schoolId`/`lessonSlotId`/`date`, per-occurrence explicit dates over slot-level/implicit ordering — confirmed design choice via `AskUserQuestion`). `CurriculumService.scheduleLesson()` links one dated `LessonSlot` occurrence to a `CurriculumTopic` (subject-match validated), reusing BK-CUR-4's academic-staff gate + `LessonSlot`-ownership check. `CurriculumService.sessionFor()` combines the slot, its scheduled topic (nullable — a slot without a plan is a real state), and the expected roster (`SchoolStore.listStudents` filtered to the slot's class section) — the concrete answer to "teachers fed expected attendees." New `POST /curriculum/schedule` and `GET /curriculum/session/{lessonSlotId}/{date}` routes, `V18` migration with a real unique index. |
...
| O6 | Reconcile `AssessmentStructure`/`GradeEntry.term` (e.g. `"2026-T1"`) against `CurriculumTopic.academicYear`/`term` (e.g. `"2026/27"`/`"Term 1"`) — no shared period-identifier convention exists yet, surfaced building BK-CUR-3 | Real (non-placeholder) wiring of curriculum coverage into BK-ASS-5/6 | Eng — likely a shared `AcademicPeriod` value type, not yet designed |
...
| 2026-09-21 | BK-CUR-2 and BK-CUR-3 closed same day (`74c31fd`), on direct instruction to continue past the original five-item pass. See their own rows above. Surfaced a genuine new open item (O6): no shared period-identifier convention exists between Assessment's `term` strings and Curriculum's `academicYear`/`term` pair, so BK-CUR-3's coverage signal is real and computable but not yet wired into the actual SPI/TPI publish flow — parked for whoever builds BK-ASS-5/6, not guessed. Only BK-CUR-4 (teacher-scoped roadmap view) remains open in Epic 5b, still gated on the same "academic-track RBAC" question BK-CUR-1 already flagged. |
| 2026-09-21 | BK-CUR-4 closed (`f2d16dd`), on direct instruction to continue further. **Epic 5b (Curriculum & Learning Roadmap) is now fully built** — BK-CUR-1/2/3/4 all done. Only O6 (Assessment/Curriculum period-identifier mismatch) and O1 (SPI/TPI formulae) still block a *real* (non-placeholder) SPI/TPI computation from actually using this epic's data. |
| 2026-09-22 | BK-ATT-2's FR-36 mark-code table formally proposed (`docs/BK-ATT-2_Mark_Code_Table_Proposal.md`), from the BuzzMe SRS gap analysis's recommendation #3. Real caveat found before recommending adoption wholesale: FR-36's table is UK DfE-style codes, not verified against Nigeria's actual regulatory scheme — proposes adopting the shape now, keeping the specific codes an explicit placeholder, staying BLK on O4 for the real Nigerian source. |
| 2026-09-22 | BK-ATT-2 closed (`fish-school-admissions` `8170adf`), on direct go-ahead superseding the proposal's own placeholder caution: "go with the flow of the UK system as at now... if and when there is a Nigerian code, we will separate the education jurisdictions." `AttendanceCode` is now the real 9-value UK table; `V17` migration backfills persisted `ABSENT` rows to `UNEXPLAINED`. O4 stays open (no Nigerian source yet) but no longer blocks. Next: `ScheduledLesson` (per-occurrence lesson-to-curriculum scheduling) per three new direct requirements — lessons scheduled to cover curriculum, teacher+students of a session known, teachers fed expected attendees. |
| 2026-09-22 | BK-CUR-5 closed (`fish-school-admissions` `9ca801b`, `c02d592`), completing the same day's second authorized piece of work. New `ScheduledLesson`/`CurriculumService.scheduleLesson()`/`sessionFor()` — per-occurrence, explicit dates confirmed via `AskUserQuestion` over slot-level/implicit ordering. Full unit + real-Postgres integration coverage (3x rerun), `V18` migration. Both mark-code-table and lesson-scheduling `COORDINATION.md` claims cleared. |
| 2026-09-22 | `GET /schools/{schoolId}/students` added (`383fdb5`), for FiSH+ER WEB's new Student Management Console (roster view, first slice). Existing `SchoolStore.listStudents()` had no HTTP route. Gated `canManageRegister`, same as the other bulk/administrative student routes. |
| 2026-09-22 | BK-PLT-8 (institution↔School linkage) and Guardian-as-Customer designed together (`docs/SchoolAdmissions_Institution_Linkage_And_Guardian_Billing_Design.md`), surfaced via FiSH+ER WEB relaying two real backend gaps found building the Console. Confirmed directly with Femi via `AskUserQuestion`: EA auto-provisions SchoolAdmissions (Recommended option) rather than a manual-link endpoint; "a Guardian is equivalent to a Customer... the service provided by a school is to TEACH his wards, it is a contract the school has to fulfil" — Guardian will become a real SOP `Customer`, sequenced *after* linkage lands (needs a real `entityId`/`organisationId` to scope the Customer to). Same-episode terminology correction: "Company" is too narrow — "a Company is a type of organisation," "a school is a type of organisation" — Company and School are siblings under a broader Organisation concept. Checked before using it anywhere: `Company`/`CompanyId` spans 250+ files across GL/EA/SOP/POP/IM/HR/WEB, so nothing was renamed unilaterally; the new linking field is named `organisationId` rather than `companyId` going forward, resolving to EA's existing `Company` row for now. BK-PLT-8's SchoolAdmissions side (`School` aggregate, idempotent `POST /schools`, new EA service-account trust) built same day, `fish-school-admissions` `57ad955` — see its own row above. EA's own provisioning call proposed to the live EA session, which is independently confirming the naming decision with Femi before committing to new Terraform/Cognito work rather than building on a relay — correctly cautious, not yet built. Guardian-as-Customer design-only, queued behind BK-PLT-8's EA-side completion. |
| 2026-09-23 | BK-PLT-8 closed end to end — real Cognito service-account login + a real production `POST /schools` call, confirmed via a temporary diagnostic route added and removed same day. One cleanup item flagged, not silently left: a real "EA Verification Test - DELETE ME" School row now exists in production with no route to remove it. On direct instruction ("build all of it now, this session"), immediately went on to build BK-FEE-5 (Guardian-as-Customer) too — new `GuardianAccount` aggregate, `FeeService.resolveGuardianCustomer`, and SOP's first-ever inbound service-account caller (`fish-sales-order-processing` `69a0d6e`), mirroring the proven POP/IM/HR pattern exactly. Two real corrections made while building, not assumed away: `relationship` stays on the per-student embedded `Guardian` (a join property, not a shared-identity one — the original design sketch got this wrong), and the whole promotion is additive rather than a migration of the existing pipe/semicolon-escaped `guardians_json` column, since the actual ask (billing rollup) doesn't need one. Full unit + real-Postgres integration coverage (3x rerun) throughout; not yet deployed to production (new Cognito/Terraform still needed on both sides). Both `COORDINATION.md` claims cleared. |
| 2026-09-23 | **Correction from FiSH+ER WEB**: EA's own calling-side commit (`4243574`) and its diagnostic-route verification, both cited above as part of "BK-PLT-8 closed end to end," are WEB's own pre-existing work from the day before — not something this thread's proposal to the EA session caused to be built, as the entry above implied. Verified via `git log` timestamps (EA's commits are 2026-09-22, same day as but after this thread's own `57ad955`) — timing alone couldn't distinguish "built in response to the proposal" from "already existed and happened to match," and WEB's specific first-hand claim ("I made and pushed it") is the more reliable account. The underlying state claim is unaffected — the linkage is genuinely live and genuinely verified — only the causation/authorship framing was wrong. BK-FEE-5 (Guardian-as-Customer) is unaffected by this correction: WEB independently verified both its commits are real, dated 2026-09-23, and pass tests, confirming that half of the report as genuinely this thread's own new work. |
| 2026-09-23 | Full code review (`/code-review`, high effort, 8 finder angles) of this session's whole SchoolAdmissions diff (10 commits, `8170adf~1..bf08450`) — 10 findings, most severe: `attendance_records.code` is `varchar(16)` but the new `AttendanceCode` enum has a 17-char value (`EDUCATIONAL_VISIT`), which will fail at the database (still open); `ParentGatewayService.childSummary`'s present/absent/late split silently drops `EDUCATIONAL_VISIT`/`OFF_SITE` from all three buckets, undercounting a guardian's real attendance total (still open); a TOCTOU race in `FeeService.resolveGuardianCustomer`/`withCustomerId` can create duplicate SOP `Customer` records for one guardian under concurrent billing (still open). Two findings resolved same day after discussion with EA: `POST /schools` splitting on an unescaped `\|` broke on institution names containing that character — **fixed** (`fish-school-admissions` `d5452cc`, `split(limit = 3)` so the name field absorbs any trailing `\|`); the caller-supplied `tenantId` having no cross-check against `organisationId` — **deliberately left as-is**, EA's own reasoned call: SchoolAdmissions has no independent channel to validate the pairing beyond re-trusting what EA already asserts, so a "fix" here wouldn't be real defense-in-depth; if this needs closing, it belongs on EA's own side (validating the pairing against its own Company/Tenant records before the call goes out). Remaining 5 findings are cleanup/efficiency (a duplicated idempotent-insert pattern across 4 new stores, `FeeService`'s constructor picking up billing-only dependencies, two N+1-shaped queries in `CurriculumService`, a redundant SELECT-before-UPDATE) — not yet triaged for action. |
| 2026-09-23 | The two most severe open findings from the review above — **both fixed** (`fish-school-admissions` `7a5ceec`). `attendance_records.code` widened `varchar(16)` → `varchar(32)` (`V22`, real headroom not just the exact minimum). `ParentGatewayService.childSummary`'s bucketing corrected: `present` now means "counts as attending (`statutoryCategory == PRESENT`) and isn't `LATE`" rather than only the literal `PRESENT` code, so `EDUCATIONAL_VISIT`/`OFF_SITE` land in `present` and `present+absent+late` sums to the real record count again. FiSH+ER WEB had flagged these as deferred (mid an unrelated EA RBAC deploy), not as "wait for me" — fixed directly with a `COORDINATION.md` claim to avoid a collision on their return. New tests cover a mixed-code summary (all three buckets sum to total) and `EXCLUDED` landing correctly in `absent`. Full unit + real-Postgres integration coverage (3x rerun). Remaining open: the SOP-Customer TOCTOU race and the 5 cleanup/efficiency findings, untouched. |
| 2026-09-23 | The SOP-Customer TOCTOU race — **fixed** (`fish-school-admissions` `d63be27`). Root cause was on the SchoolAdmissions calling side, not SOP itself: two concurrent `FeeService.resolveGuardianCustomer` calls for the same guardian could both observe no `customerId` yet, both call SOP's `POST /customers`, and `GuardianAccountStore.withCustomerId`'s old unconditional `UPDATE` let whichever write landed last silently overwrite the other. `withCustomerId` is now a compare-and-swap (`WHERE customer_id IS NULL`, both Exposed and InMemory), always re-reading the row afterward so every caller converges on the same winning `customerId` rather than each keeping its own — the loser's SOP `Customer` still goes unreferenced (pre-claiming before the SOP call would be real extra machinery for a narrow, rare window), a bounded cosmetic cost now, not a correctness bug. New tests: a semantic CAS test (unit + real-Postgres), and a genuine concurrency test forcing coroutine interleaving across 5 parallel calls that provably race (`callCount > 1`) yet converge on exactly one `customerId`. Coordinated with FiSH+ER WEB and EA before starting (no overlap, go-ahead given); `COORDINATION.md` claimed and cleared. Only the 5 cleanup/efficiency findings from the review remain open. |
| 2026-09-23 | The 5 remaining cleanup/efficiency findings triaged: `withCustomerId`'s redundant-SELECT finding is **moot** (the TOCTOU fix above already restructured it to UPDATE-then-SELECT, and the SELECT is no longer redundant — it's how each caller learns who won); the `runBlocking` finding **parked** (a verbatim copy of a pattern already live in SOP/IM/HR's own token providers — a SchoolAdmissions-only fix would create inconsistency without closing the real platform-wide tradeoff); the duplicated insert-then-unique-violation-retry pattern and `FeeService`'s billing-only constructor deps **deferred** (real but cosmetic, zero correctness risk, no urgency). The one worth doing — `scheduleLesson`/`sessionFor`'s N+1-shaped full-school-then-filter queries — **fixed** (`fish-school-admissions` `0aa25ba`): new `SchoolStore.lessonById(schoolId, lessonSlotId)`, an O(1) map lookup in-memory and a real indexed `WHERE id = ?` in Exposed, replaces both call sites (the same anti-pattern a 2026-09-20 review already fixed once for `AttendanceService.mark()`). New unit + real-Postgres integration tests (3x rerun). All 10 findings from the original review are now accounted for: 5 fixed (`varchar(16)` column, Parent Gateway bucketing, the TOCTOU race, the pipe-delimiter, and this N+1 fix), 1 deliberately left as EA's own trust-boundary call (`tenantId`), 1 parked as a platform-wide (not SchoolAdmissions-only) concern (`runBlocking`), 2 deferred as real-but-cosmetic cleanup (the duplicated insert pattern, `FeeService`'s constructor bloat), and 1 resolved as moot by the TOCTOU fix (the redundant SELECT). |
| 2026-09-23 | One of the 2 deferred cleanup findings tackled — the duplicated insert-then-unique-violation-retry pattern, **fixed on the Exposed (production) side** (`fish-school-admissions` `795c8a3`). New `UniqueViolations.insertIdempotent(find, insert, conflictMessage)` in `TenantIdGuard.kt` (this codebase's existing shared home for tenancy/unique-violation persistence helpers) collapses `ExposedSchoolRegistryStore.provision` and `ExposedGuardianAccountStore.resolveOrCreate`'s hand-copied "find first, try insert, on race re-find and return the winner" shape into one place. Semantically identical to the old code (same double-find-on-race behaviour, same lazy id generation) — proven by both stores' existing real-Postgres conflict tests passing unchanged (3x rerun). **Deliberately scoped to Exposed only**, not the two InMemory equivalents: they're test-only code, correctly implemented, and structurally *less* identical (`GuardianAccountStore` dedupes by two natural keys, `SchoolRegistryStore` by one) — a generic abstraction there would add complexity rather than remove it, so left as three similar lines rather than a premature one. `FeeService`'s constructor-bloat finding remains the one open deferred item. |
| 2026-09-23 | The last deferred cleanup finding tackled — `FeeService`'s billing-only constructor dependencies, **fixed** (`fish-school-admissions` `ec41b6b`). New `GuardianBillingService(guardianAccountStore, schoolRegistry, sopCustomerGateway)` holds `resolveGuardianCustomer` (moved verbatim) — `FeeService` keeps `guardianAccountStore` alone, the one dependency that genuinely is its own concern (validating `issueInvoice`'s optional `guardianAccountId` reference actually exists). `AppServices` gains a `guardianBilling` field; the `/fees/guardians/resolve` route now calls it instead of `services.fees`. Tests split accordingly: `GuardianBillingServiceTest.kt` (all `resolveGuardianCustomer`/SOP-integration tests, including the concurrency test) and `FeeServiceGuardianAccountIdTest.kt` (`issueInvoice`'s own guardian-reference validation, now fully decoupled from SOP). Pure application-layer refactor — no persistence/SQL changes, so no new Postgres verification needed; full unit suite green. **All 10 findings from the original code review are now closed out**: 7 fixed (`varchar(16)` column, Parent Gateway bucketing, the TOCTOU race, the pipe-delimiter, the duplicated insert pattern, the N+1 queries, and this constructor-bloat fix), 1 left as EA's own deliberate trust-boundary call (`tenantId`), 1 parked as a platform-wide concern (`runBlocking`), and 1 resolved as moot by the TOCTOU fix (the redundant SELECT). |

...
| BK-REG-5 | — (BuzzMe UC-20 follow-on) | DPID identity linkage: `dpidIdentityId` field on `StudentProfile` + reverse-lookup route so DPID's `PrincipalStudentDirectory.resolveStudentId()` can resolve a real student | M | BK-REG-1; unlocks BK-ATT-6's remaining DPID-side gap. BuzzMe's Open Question 1 resolved 2026-09-20 (lookup call to Principal, not a DPID-side cross-reference) — `externalRef` confirmed not reusable (load-bearing for `AdmissionsService`'s duplicate-enrolment check). Proposed shape from BuzzMe, ours to finalize. Note: populating the field (at credential-issuance time) is a separate, still-unbuilt step on BuzzMe's own side — this closes the read-side lookup only. |
...
| BK-CLS-3 | FR-CLS-3 | Mid-year class transfers with history | S | BK-CLS-1, BK-REG-1 |
...
| BK-ADM-5 | FR-ADM-3 | Offer letters + guardian accept/decline | S | BK-ADM-1 |
...
| BK-ATT-2 | FR-ATT-3 | Government-mandated attendance codes only; reject others | M | **BLK:** attach official code list |
...
|----|------|-----|-----------------|
| BK-CUR-1 | Subject curriculum map per school/year (topics, sequence, term coverage) | S | BK-CLS-*; parked until post-W4 capacity |
...
| ID | SRS | Item | Pri | Deps / blockers |
|----|-----|------|-----|-----------------|
| BK-CMP-1 | FR-CMP-1 | Immutable audit log for pastoral/safeguarding notes | M | NFR-SEC-2 |
| BK-CMP-2 | FR-CMP-4 | RBAC on sensitive fields across all modules (API-enforced) | M | BK-PLT-2, NFR-SEC-3 |

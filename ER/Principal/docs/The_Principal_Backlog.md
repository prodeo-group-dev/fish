...
| BK-PLT-8 | Real institution↔School linkage (EA auto-provisioning) | M | SchoolAdmissions side **done 2026-09-22** (`fish-school-admissions` `57ad955`) — new `School` aggregate, idempotent `POST /schools`, new EA service-account trust (`requireServiceAccount`, `buildJwksServiceVerifierForEa`). Design: `docs/SchoolAdmissions_Institution_Linkage_And_Guardian_Billing_Design.md`. **EA's own call-out side not built yet** — proposed to the live EA session, which is independently confirming the Organisation/Company naming decision with Femi first. Not deployed to production either (`SCHOOLADMISSIONS_JWT_SERVICE_AUDIENCE_EA` unset until EA's Cognito client exists). Replaces WEB's Student Management Console manual-paste-School-ID stopgap once EA's side lands. Precedes Guardian-as-Customer (needs a real `organisationId` to scope SOP's `Customer.entityId` to). |

...
| **3. Transaction Price Determination** | `TransactionPrice` | `SalesOrderLine.unitPrice` × `SalesOrder.totalAmount` | **Real gap.** `totalAmount` is a flat sum of line amounts. No variable consideration (rebates, discounts, returns), no non-cash consideration (IFRS 13 fair value), no constraint mechanism on variable consideration at all. If the real trade finance deals ever include rebate clauses or non-cash settlement, this needs new modeling — nothing here today handles it. |

...
**End-to-end wiring — built, 2026-08-21.** Both `RecordSaleUseCase`/`RecordCollectionUseCase` are now real GL Engine HTTP routes (`POST /sales/record-sale`/`POST /sales/record-collection`), and SOP has its own `application`/`infrastructure.gl` layers actually calling them: `SalesOrder.invoice()` (a new domain method — UC-SO5's trigger point, previously undefined despite `SalesOrderStatus` already naming `INVOICED`) pairs with SOP's own `RecordSaleUseCase`; `BillForCollection.collect()` pairs with SOP's own `RecordCollectionUseCase`. Neither GL Engine call rolls back its SOP-side domain transition on failure — the same eventual-consistency tradeoff §2 already named, not a new one. `companyId`/tenant scoping on the GL routes departs from every other route in that repo (caller-supplied in the body, since neither use case has an owning aggregate to resolve it from) — confirmed with the user before building.
...
- `salesOrderId`, `confirmedBy: ConfirmingAuthority` (`SALES_ADMIN`/`CFO_ACCOUNTANT`/`BANK` — UC-SO3's actor list, typed rather than free text), `confirmedAt`, `isOverride`, `overrideRationale?`. A record of *when and by whom* the gate was satisfied — NFR-SO02's traceability requirement needs this to be its own fact, not just a boolean flipped on `SalesOrder`. `evidenceReference` (documentary evidence of the aval) remains unbuilt, not needed to satisfy NFR-SO01/02.
- **Override path — built.** Same `AvalConfirmation` shape with an `isOverride: Boolean` flag (§5 item 10, resolved: not a distinct concept). `SalesOrder.confirmAval()` enforces both of NFR-SO01's requirements as `require()` checks: override authority must be `ConfirmingAuthority.CFO_ACCOUNTANT` specifically (Sales Admin/Bank can confirm normally but cannot self-authorize a bypass), and `overrideRationale` must be non-blank whenever `isOverride` is true. Treated as a caller-contract violation (throws), not a gracefully-rejectable domain event — the calling system's own authorization layer is expected to have already verified the actor's role before invoking the override path at all.

### 3.5 `BillForCollection`
- `salesOrderId`, `presentedAt`, `status: Presented | Accepted | Due | Collected`, `avalisingBank?`, `dueDate?`, `collectedAmount?`, `headroomRestored: Boolean`. Drives which GL Engine account the *Record collection* call credits/debits (§0/§4).
- **`FacilityHeadroom` coordination — built.** `restoreHeadroom(headroom, restoreAmount)` requires `Collected` status and prevents double-restoration, but deliberately takes `restoreAmount` as an explicit caller-supplied argument rather than defaulting to `collectedAmount` — the exact restoration rule (§3.6/§5 item 4) is still open, so this method only enforces *that* restoration can happen and only once, never *how much*.
...
3. **Aval-override governance (§3.4) — partially resolved.** The codeable floor is built and enforced: override authority must be `ConfirmingAuthority.CFO_ACCOUNTANT`, matching NFR-SO01's own stated minimum. Which *specific named individual or title* beyond that generic role-level description holds override authority remains a business decision this document doesn't resolve, same category as item 1.
...
10. **`AvalConfirmation`/override modeling (§3.4) — resolved, built.** Same shape with an `isOverride` flag plus `overrideRationale`; see §3.4's updated entry.
...
## 7. Reconciliation against the IFRS 15 5-step model (2026-08-21)

`docs/Sales_Processing_Requirements_Specification.md` §3 describes a fuller, more formal decomposition of revenue recognition than anything built here: `ContractRecord`, `PerformanceObligation[]`, `TransactionPrice`, `PriceAllocation[]`, `RevenueEvent`. Per that document's own status note, this is a broader layer SOP's trade-finance scenario is *one case of*, not a replacement for SOP's design. This section maps SOP's actual model onto that 5-step decomposition, step by step, distinguishing genuine gaps from deliberate simplifications appropriate to SOP's scenario.

| IFRS 15 step | Spec's entity | SOP's equivalent | Assessment |
|---|---|---|---|
| **1. Contract & Sales Order Recognition** | `ContractRecord` + `SalesOrder` | `SalesOrder` (`customerId`, `entityId`, `facilityId?`, `deliveryTerms`, `lines`) | **Mostly reconciled, one open scoping question.** `SalesOrder` already captures customer identity, delivery terms, and (via lines) pricing. Missing: a structured contract-type distinction (spot/forward/CIF/FOB/EXW) — `deliveryTerms` is a free-text `String` (e.g. "CIF Conakry"), not a typed enum; a "commercial substance/enforceable rights" validation step doesn't exist (arguably a human/compliance sign-off, not a domain-model concern, the same category as UC-SO3's own aval-confirmation-status open item — a business fact, not a design gap). **Genuinely open:** does SOP ever need a `ContractRecord` distinct from `SalesOrder` — e.g. one master agreement spanning multiple orders/draw-downs — or is `SalesOrder` itself always the right unit? Not guessed here. |
| **2. Performance Obligation Identification** | `PerformanceObligation[]` | `SalesOrderLine` | **Real gap, scoped to what SOP doesn't need yet.** Each `SalesOrderLine` is *implicitly* treated as one distinct performance obligation (its own price, its own fulfilment tracking via `fulfillLine()`), which is correct for SOP's scenario (goods-only lines, no bundling). But there's no explicit obligation-type field (goods vs. warranty vs. service) and no support for **composite/bundled obligations** (e.g. goods + logistics priced as one line) — the spec's §3.2 explicitly calls for both. Not built because the sugar-deal scenario doesn't need them, not because they're hard. |
| **3. Transaction Price Determination** | `TransactionPrice` | `SalesOrder.transactionPrice` (`TransactionPrice`, `VariableConsiderationComponent`) | **Variable consideration built, 2026-08-21 — non-cash consideration still open.** `transactionPrice.total` = `totalAmount` (base price) adjusted by every recorded rebate/discount/refund's *constrained* amount (IFRS 15.56-58's constraint: `constrainedAmount` can only shrink an `estimatedAmount` toward zero, never exceed or reverse it — enforced, not computed, since the underlying probability judgment is a business call, not something code can determine). `addVariableConsideration()` locks once `Invoiced`, since the price must be settled before revenue recognition fires. `RecordSaleUseCase` now posts `transactionPrice.total`, not the flat `totalAmount`. IFRS 13 non-cash consideration is **not** built — narrower scope than this row originally flagged, deliberately not built speculatively alongside variable consideration since it wasn't asked for. |
| **4. Allocation of Transaction Price** | `PriceAllocation[]` | *(none — currently moot)* | **Not a gap yet, but linked to Step 2's gap.** Allocation only matters when a transaction price is bundled across obligations and needs splitting by standalone selling price. Since SOP's lines are already priced independently (no bundling exists, per Step 2), there's nothing to allocate. This stops being true the moment composite obligations get built — the two gaps should be closed together, not separately. |
| **5. Revenue Recognition** | `RevenueEvent` | `SalesOrder.invoice()` + `RecordSaleUseCase` | **Fully reconciled, already resolved.** This is the piece §4/§5 of this document already worked through in detail — `invoice()` is the trigger, firing at the contractually correct point (CIF loading, not physical fulfilment), calling `RecordSaleUseCase` to post Dr AR/Cr Revenue. No further reconciliation needed here. |

**Net assessment:** SOP's model is a **deliberate, scenario-appropriate simplification** of the full 5-step decomposition, not an incomplete or incorrect implementation of it. Step 5 (the step this document's own §4 resolution work concentrated on) is fully reconciled. Steps 3 and 4 (variable/non-cash consideration, price allocation) and part of Step 2 (composite obligations) are the genuine gaps — all three trace back to one root cause: **SOP has never needed a bundled or variable-price scenario**, because the sugar-deal scenario it was built against doesn't have one. Building `PerformanceObligation[]`/`TransactionPrice`/`PriceAllocation[]` speculatively, without a concrete deal that needs them, isn't done here — per `[[feedback_park_dont_guess]]`, this section identifies the gaps precisely rather than guessing whether they're worth closing now.

**Open question surfaced by this reconciliation, not resolved:** does any real, current, or near-term deal actually need variable consideration, non-cash consideration, bundled obligations, or a multi-order `ContractRecord`? If not, none of this blocks anything — SOP's simpler model remains correct for its scope. If yes, that's the trigger for the next SOP increment, not a speculative build now.

...
3. ~~**Exception messages leak to API clients.**~~ **Resolved 2026-08-21.** `StatusPages`' catch-all now logs the real exception server-side (`call.application.log.error`, with method+path context) and returns only a generic `"internal_error"` to the client — `cause.message` no longer reaches the response body. Covered by `StatusPagesTest`.

### High

4. ~~**No structured logging configuration.**~~ **Resolved Platform-wide 2026-09-03.** JSON console output via `logstash-logback-encoder`, plus a per-request `requestId` in every log line via Ktor's `CallId` plugin. Confirmed live in CloudWatch for `GL/`; ported identically to `POP/`, `SOP/`, `IM/`, and `HR/` the same day (each needed the `ktor-server-call-id` dependency added alongside `ktor-server-call-logging` — not obvious from `GL/`'s own build file alone, since it already had it from an earlier increment).

...
- **Now plans against real AWS** (2026-08-26) — moved off root credentials (surfaced when `aws login` authenticated as the account root user) to a scoped `fish-gl-engine-terraform` IAM user with a least-privilege customer-managed policy. A real `terraform plan` reached 16 planned resources before stopping on a genuinely missing permission (`ec2:DescribeVpcAttribute`) — fixed in the policy file, but pushing the new policy version needs a privileged identity, since the scoped user correctly can't modify its own permissions. **Still no successful `terraform apply`.**
- **HTTPS closed** (2026-08-26) — domain confirmed: `capital.theprodeogroup.com`. ACM certificate (DNS-validated) plus an HTTPS listener, HTTP now redirects instead of forwarding directly. theprodeogroup.com's DNS is external (not Route 53 in this AWS account), so this is a genuine two-phase `apply` with manual DNS steps in between — see `infra/terraform/README.md`'s setup steps. Not yet exercised end-to-end (blocked on the same missing-permission issue above).

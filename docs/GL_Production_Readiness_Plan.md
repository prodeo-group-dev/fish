...
- **`terraform fmt`/`init`/`validate` now actually run and pass** (2026-08-22, once Terraform was installed) — and `validate` earned its keep immediately: caught a hand-typed GitHub OIDC thumbprint that was only 39 hex characters, not the required 40. Fixed properly, not patched — replaced the hardcoded value with a `data "tls_certificate"` fetch of GitHub's real certificate at `apply` time, removing the whole "did I copy this right / has GitHub rotated it" class of risk.
- **`terraform plan` now genuinely succeeds against real AWS** (2026-08-26) — moved off root credentials (surfaced when `aws login` authenticated as the account root user) to a scoped `fish-gl-engine-terraform` IAM user with a least-privilege customer-managed policy. First `plan` reached 16 planned resources before stopping on a genuinely missing permission (`ec2:DescribeVpcAttribute`) — fixed in the policy file; pushing the new policy version needed a privileged identity (a second, separate one-time root session, since the scoped user correctly can't modify its own permissions). Re-ran clean: **23 resources planned, 0 errors** — every resource in the config (ECR, IAM/OIDC, Secrets Manager, CloudWatch Logs, ECS cluster/task definition/service, both security groups, the ALB with HTTP/HTTPS listeners, the ACM certificate) resolves. **Still no successful `terraform apply`** — `plan` succeeding is a real, meaningful signal, but nothing has actually been created in AWS yet beyond the IAM bootstrap itself.
- **HTTPS closed** (2026-08-26) — domain confirmed: `capital.theprodeogroup.com`. ACM certificate (DNS-validated) plus an HTTPS listener, HTTP now redirects instead of forwarding directly. theprodeogroup.com's DNS is external (not Route 53 in this AWS account), so `apply` is a genuine two-phase process with manual DNS steps in between — see `infra/terraform/README.md`'s setup steps.

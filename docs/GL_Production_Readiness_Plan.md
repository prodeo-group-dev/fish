...
## Status as of 2026-08-26 (fourth revision)

Since the third revision: **Phase 1b (CD pipeline, finding #1) went from "built, unverified" to actually applied and running in AWS**, database provisioning (previously the one deliberately-out-of-scope item) is closed, and the GitHub Actions repository variables are set for real — see Phase 1b below for the full account. Nothing else in this plan has moved.
...
5. **CI itself stopped triggering entirely, discovered and worked around 2026-08-26 — separate from the Phase 1b build above, and still not fully resolved.** After run #25 (2026-08-21 23:43, commit `b7d448d`), the `ci.yml` workflow (id `338476737`) stopped producing any run at all, on any trigger (push, a web-UI-direct commit, and a fresh PR all confirmed dead via the GitHub web UI — `gh run list`/`gh api` were separately found to be giving stale/wrong data all session and are not trustworthy for this). A brand-new workflow file triggers immediately where the old one doesn't, isolating it to that specific workflow registration being stuck on GitHub's side — so the pipeline was re-registered as `pipeline.yml` (commit `26e5656` in `GL/`). That first replacement then also failed to trigger on two more pushes, and a run that briefly appeared to succeed under an interim diagnostic file later vanished from history even after a hard refresh — so this isn't fully explained as "one stuck workflow ID." A GitHub Support ticket has been filed; **CI/CD should be treated as unverified-in-practice until a run is actually confirmed on `pipeline.yml`**, not assumed working from the Phase 1b build above.

...
- **Unverified against real AWS** — written and reviewed, never run through `terraform validate`/`plan`/`apply` (no CLI, no credentials in the environment it was built in). The first real signal comes from someone actually running `terraform apply` and pushing to `master`.

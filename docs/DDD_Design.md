...
- **Depreciation, flagged, not designed or built.** Fixed Assets are "likely to incur depreciation charges over time depending on how long is required by the system user" — i.e., useful life is a user-configurable input, not a hardcoded rule. Real scope this implies, none of it built: a depreciation method and useful-life period need to live *somewhere* per Fixed Asset — either new fields directly on `Account`, or (following the same pattern already established for AP/AR, Section 2.5) a **Fixed Asset Register** subsidiary ledger, with a control account in Ledger and per-asset detail (cost, useful life, method, accumulated depreciation) owned by whichever module manages it. Periodic depreciation postings would use `JournalSource.SYSTEM` — already anticipated in that enum's own original KDoc example ("Monthly depreciation entry"), written before this session even started. Which shape (fields on `Account` vs. a new control-account module) isn't decided — don't build either without asking.
...
**Company** (aggregate root, referenced by ID from Ledger, not embedded in Tenant) — one legal entity, its own COA and books. Carries a `goingConcernStatus` (`GoingConcernStatus` — resolved 2026-08-11, see Section 9.7): every Company defaults to `ASSUMED` going concern at onboarding — this is the standard accounting default, not a special case — and can later move to `SUBSTANTIAL_DOUBT`, triggered from the Lending context (Section 3.3) rather than invented as a separate mechanism.

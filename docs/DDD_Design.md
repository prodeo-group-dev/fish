...
5. ~~Work through Section 6 in order — `Money` first (red → green), then `Account`, then `Period`, then `JournalEntry`.~~ **Done (2026-08-11).** All four built and tested, 81 passing tests total. `CashBookEntry` (Section 3.1) is now the natural next build — the feature that originally motivated starting this whole build order — followed by the PO/SO/Payroll/Inventory ecosystem (Section 2.5–2.8) or `ArrearsCase`/`EquityStake` for Lending, depending on priority.

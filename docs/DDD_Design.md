...
- **New requirement, 2026-08-11, per IAS 1 — not yet in the original design, add before building:** Asset and Liability accounts need a **current vs. non-current classification**, not just the five base types. IAS 1's rule (the user's own citation): a liability is *current* unless the Company has the unconditional right to defer settlement for ≥12 months — and a covenant breach can force an otherwise non-current liability into current classification (repayable on demand). Most of what FiSH will post early on (trade payables/receivables via Purchase/Sales Order Processing, Section 2.5) will be current in practice, but the classification still needs to be a real field on `Account`, not assumed — this is what a balance sheet's current/non-current split (a standard IAS 1 presentation requirement, not an edge case) depends on. Covenant-triggered reclassification specifically is a much narrower, likely-later concern (more relevant to term debt/Scrip's treasury book than everyday trade payables) — don't build that mechanism now, but don't design the base classification field as if it doesn't need to exist.

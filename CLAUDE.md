...
This is two git repos, not one: `FiSH/` (this repo — docs, business plans, `CLAUDE.md`) and `GL/` (the Kotlin engine code), pushed separately to `https://github.com/prodeo-group-dev/fish` and `https://github.com/prodeo-group-dev/fish-fish-gl-engine`. `GL/` is linked into `FiSH/` as a **git submodule** (`.gitmodules`), pinned to a specific commit — cloning `FiSH/` needs `git clone --recurse-submodules` (or `git submodule update --init` after a plain clone) to actually get the code, and after pulling changes made *inside* `GL/`, the submodule pointer in `FiSH/` needs its own commit to move forward (it doesn't update automatically). Don't assume editing files under `GL/` and committing in `FiSH/` is enough — `GL/` commits happen in its own repo first, `FiSH/` only records which `GL/` commit it's pinned to.
